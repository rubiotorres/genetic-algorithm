...
Vol[j] vale o valor do volume da embalagem do produto j, e o frete total da loja i é a soma dos volumes 
(de todos os produtos comprados na loja i) multiplicada por Fi mais o valor base (= 23.5), e se o valor 
gasto (custo de preço e nao de frete) numa loja i atingir Vi, o frete total se torna gratuito
"""
import math
...
class Genetico(object):
	...
	F: array-like 	shape(L)
	Fi - valor que a loja i cobra no frete para cada litro de volume

	Vol: array-like shape(N)
	Vol[j] - volume da embalagem do produto j
	...
	def __init__(self, pop_size=100, crc=0.7, alpha=0.5, n_iter=1000, cmm=0.01, mut_interval=10.0, k=1):
		...
		'''self.F = np.random.rand(self.L)
		for a in np.nditer(self.F, op_flags=['readwrite']):
			a[...] = a * 10 + 5'''
		self.F = np.array([13.11526584,  7.21772406, 11.67223324,  7.13590286, 10.74118828])
		print 'F', self.F

		'''self.Vol = np.random.random(self.N)
		for a in np.nditer(self.F, op_flags=['readwrite']):
			a[...] = a * 4.5 + 0.5'''
		self.Vol = np.array([0.18928367, 0.40332937, 0.29860336, 0.0501233,  0.18716994, 0.79069656, 0.08843891, 0.07365922, 0.79391972, 0.87244253])
		print 'Vol', self.Vol
	...
	def calcula_custo_total(self, X):
		...
		for i in range(self.L):
			...
			for j in range(self.N):
				...
				volume_total += X[i][j]*self.Vol[j]
			
			if custo_preco >= self.V[i]:				
				custo_total += custo_preco
			else:
				custo_frete = 23.5 + volume_total * self.F[i]
		...
		for k in range(self.n_iter):	
			...
			for i in range(int(self.pop_size/2)):
				...
				if chance_reproducao >= self.crc:
					...
					cada um dos 2 filho é definido pela formula, que define o i-esimo atributo do filho de acordo com o i-esimo atributo do pai 1 e com o i-esimo atributo do pai 2
					F1 = alpha*P1 + (1-alpha)*P2
					F2 = alpha*P2 + (1-alpha)*P1
